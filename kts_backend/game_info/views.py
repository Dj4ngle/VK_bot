...
from kts_backend.game_info.schemes import (
    ListQuestionSchema,
    QuestionSchema, QuestionEditeSchema, SessionSchema, ListQSessionSchema,
)
...
class QuestionAddView(AuthRequiredMixin, View):
    ...
    async def post(self):
        question = await self.store.game_info.create_question(self.data["title"], self.data["answer"])
        if not question:
            raise HTTPConflict
...
class SessionsView(AuthRequiredMixin, View):
    ...
    async def get(self):
        ...
        return json_response(data=ListQSessionSchema().dump({"sessions": sessions}))
