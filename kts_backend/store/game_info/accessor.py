...
from kts_backend.game_info.models import Question, QuestionModel, Sessions, SessionsModel
...
class QuizAccessor(BaseAccessor):
    ...
    async def create_question(self, title: str, answer: str) -> Question:
        async with self.app.database.session() as session:
            try:
                res = await session.execute(insert(QuestionModel).
    ...
    async def edite_question(self,
        ...
        async with self.app.database.session() as session:
            ...
            res = await session.execute(update(QuestionModel).
                                        where(QuestionModel.id == _id).
                                        values(**update_values).
                                        returning(QuestionModel.id, QuestionModel.title, QuestionModel.answer))
            await session.commit()
        ...
        return Question(id=question[0].id, title=question[0].title, answer=question[0].answer)
    ...
    async def list_session(self,
        ...
        async with self.app.database.session() as session:
            res = await session.execute(select(SessionsModel).where(**update_values))
        return res.scalars().all()
